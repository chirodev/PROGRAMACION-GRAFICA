...
namespace Juego
{
    class Pieza : Objeto
    {
        ...
        public override void escalar(float scala)
        {
            sca = scala;
        }
        public override void rotar(float angle, float x, float y, float z)
        {
            ...
            {
                Angulo.X = angle;
                model = Matrix4.CreateRotationX (Origen.X);
                
            }
            if (y > 0)
            {
                Angulo.Y = angle;
                model = Matrix4.CreateRotationY(Angulo.Y);
                
            }
            if (z > 0)
            {
                Angulo.Z = angle;
                model = Matrix4.CreateRotationZ(Angulo.Z); 
            }
        ...
        {
            traslacionPieza.X = traslacionPieza.X + trasX;
            traslacionPieza.Y = traslacionPieza.Y + trasY;
            traslacionPieza.Z = traslacionPieza.Z + trasZ;
        }
    }
}
